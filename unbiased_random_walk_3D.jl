...
for i = 2:length(x)
    ...
    # Calculate the difference in theta and phi between current step and previous step
    dtheta =
    dphi =
...
println(size(all_r))
println(size(all_theta))
println(size(all_phi))
println(size(time))
println(size(holding_time))
...
println("straightness index:", s_index)
