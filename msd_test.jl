...
# Here we have added another summary statistic = msd
# msd = mean squared displacemnt. Work out the displacement
...
for i = 1:length(walks)
    ...
    for i = 2:length(x)
        ...
        r = rand(TruncatedNormal(0.8, 0.1, 0, 1))
...
zipped_msd = zip(delta_msd, means)
...
sns.distplot(x, axlabel="Mean Step Length", color="salmon")
ax[:set_xlim]([0,1])
ax[:set_title]("Mean Step Length Posterior Distribution: msd_1")
# ------------------------------------------------------------------------------
# 6. msd_01
# for i in zipped_msd
#     if i[1] <= e_msd_01
...
# sns.distplot(x, axlabel="Mean Step Length", color="salmon")
# ax[:set_xlim]([0,1])
# ax[:set_title]("Mean Step Length Posterior Distribution: msd_01")
# ------------------------------------------------------------------------------
